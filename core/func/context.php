	...
	class context {
		...
		public static function getUserImage($userSheet) {
			if ($userSheet['imgTime'] == 0 || $userSheet['imgTime'] == null) {
				return 'https://labs.rhodum.xyz/imageServer/user/def6.png?v=2';
			}else{
				return 'https://labs.rhodum.xyz/imageServer/user/'.$userSheet['id'].'.png?time='.$userSheet['imgTime'];
			}
		}
		...
		public static function getGroupImage($userID) {
			...
			if ($userSheet['imgTime'] == 0 || $userSheet['imgTime'] == null) {
				return 'https://labs.rhodum.xyz/imageServer/user/def6.png';
			}else{
				return 'https://labs.rhodum.xyz/imageServer/user/'.$userSheet['id'].'.png?time='.$userSheet['imgTime'];
			}
		}
		...
		public static function getItemThumbnail($type, $id, $datafile, $fileHash) {
			if ($type == "hats") return 'https://labs.rhodum.xyz/imageServer/hats/'.$datafile.'.png';
			if ($type == "heads") return 'https://labs.rhodum.xyz/imageServer/heads/'.$datafile.'.png';
			if ($type == "faces") return '/data/assets/faces/texture/'.$datafile;
			if ($type == "gear") return 'https://labs.rhodum.xyz/imageServer/gear/'.$datafile.'.png';
			if ($type == "decals") return "/data/assets/uploads/".$fileHash;
			if ($type == "tshirts") return 'https://labs.rhodum.xyz/imageServer/tshirts/'.$id.'.png';
			if ($type == "pants") return 'https://labs.rhodum.xyz/imageServer/pants/'.$id.'.png';
			if ($type == "shirts") return 'https://labs.rhodum.xyz/imageServer/shirts/'.$id.'.png';
		}
		
		public static function getItemThumbnailC($type, $id, $datafile, $fileHash, $time) {
			if ($type == "hats") return 'https://labs.rhodum.xyz/imageServer/hats/'.$datafile.'.png?tick='.strtotime($time);
			if ($type == "heads") return 'https://labs.rhodum.xyz/imageServer/heads/'.$datafile.'.png?tick='.strtotime($time);
			if ($type == "faces") return '/data/assets/faces/texture/'.$datafile.'?tick='.strtotime($time);
			if ($type == "gear") return 'https://labs.rhodum.xyz/imageServer/gear/'.$datafile.'.png?tick='.strtotime($time);
			if ($type == "decals") return "/data/assets/uploads/".$fileHash;
			if ($type == "tshirts") return 'https://labs.rhodum.xyz/imageServer/tshirts/'.$id.'.png?tick='.strtotime($time);
			if ($type == "pants") return 'https://labs.rhodum.xyz/imageServer/pants/'.$id.'.png?tick='.strtotime($time);
			if ($type == "shirts") return 'https://labs.rhodum.xyz/imageServer/shirts/'.$id.'.png?tick='.strtotime($time);
		}
		...
		public static function getrenderimageCount() {
			$query = "SELECT id FROM `renders`;";
			$stmt = $GLOBALS['dbcon']->prepare($query);
			$stmt->execute();
			return $stmt->rowCount();
		...
		public static function sendDiscordMessage($message) {
			$url = "https://discord.com/api/webhooks/853010743300718602/HWXm0WB7OQ4TwKwTlG7mBl-Ze7wdb8VoO43GtilurdolmTWV6X3FcT_1rE50RoxdZClC";
		...
		public static function parseEmoticon($content) {
            ...
            $content = str_replace(":troll:",'<img height="25" width="25" src="/html/img/emoticons/troll.png">', $content);

<?php
	if (isset($_POST['csrf']) and isset($_POST['itemID'])) {
		...
		if ($result['approved'] == 0 and $result['declined'] == 0) {
			...
			if ($type != "decals") {
				$message = 'Your asset named '.$assetName.' has been approved and can be seen in the catalog. You also have received the item in your inventory. Your item can be found at http://rhodum.xyz/catalog/item/'.$itemID;
			}else{
				$message = 'Your asset named '.$assetName.' has been approved and can be seen in the catalog. Your item can be found at http://rhodum.xyz/catalog/item/'.$itemID;
			}

<?php
	if (isset($_FILES['file']) and isset($_POST['itemName']) and isset($_POST['itemDescription']) and isset($_POST['itemType']) and isset($_POST['itemPrice']) and isset($_POST['csrf_token'])) {
		...
		if ($imageFileType != "jpg" && $imageFileType != "JPG" && $imageFileType != "png" && $imageFileType != "PNG" && $imageFileType != "jpeg" && $imageFileType != "JPEG" && $mime != "image/png") {
        ...
        // use beta moderation api to moderate it
        $modurl = 'https://api.moderatecontent.com/moderate/?key=f835bcc3fa2d8ef2368b9a4d3e50f795&url=http://rhodum.xyz/data/assets/uploads/'.$fileHash;
        $json = file_get_contents($modurl);
        $obj = json_decode($json);

        // check what the api says
        if ($obj->rating_label == "adult"){
            $approved = 0;
        }
		...
		$query = "INSERT INTO messages (`recv_uid`, `sender_uid`, `title`, `content`) VALUES (:userId2, 1, :title, :msg);";

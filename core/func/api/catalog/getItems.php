	...
	if ($GLOBALS['loggedIn'] == false) {
		...
		foreach($stmt as $result) {
			...
			if ($count < 16) {
                ...
                $rarity = context::secureString($result['rarity']);
				if (strlen($itemName) >= 30) {
					$itemName = substr($itemName, 0, 27). " ... ";
				}
                if ($rarity == 1){
                    echo '<h5 style="float:left;">'.$itemName.'</h5>';
                    echo '<p style="color:#FF0000;float:left;">Limited<p>';
                }else{
                    echo '<h5 style="float:left;">'.$itemName.'</h5>';
                    echo '<p><p>';                   
                }
				...
				if ($result['type'] != "decals") {
					if ($result['currencyType'] == 0) {
						...
						echo '<h5 style="color:#158cba;float:right;"><span class="fas fa-coins"></span> '.$result['price'].'</h5><br>';
					}
				}

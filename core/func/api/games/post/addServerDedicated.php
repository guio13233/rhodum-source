<?php
	if (isset($_POST['csrf_token']) and isset($_POST['serverName']) and isset($_POST['serverDescription']) and isset($_POST['versionType']) and isset($_POST['privacyType'])) {
		...
		if (isset($_FILES['placeFile'])) {
			...
			$webDirectory = "http://rhodum.xyz/data/assets/uploads/".$fileHash;
		}else{
			if ($genPlace == 0) die("error");
			if ($genPlace == 1) $webDirectory = "http://labs.rhodum.xyz/places/baseplate.rbxl";
		}
		
        // Optimize server preformance
        $lastserverid = 1;
	    $stmt = $dbcon->prepare("SELECT Lastserver FROM ServerInfo WHERE id=:id;");
	    $stmt->bindParam(':id', $lastserverid, PDO::PARAM_INT);
	    $stmt->execute();
	    $result = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($result['Lastserver'] == 1){
            $serveroptimize = 2;
        } elseif($result['Lastserver'] == 2){
            $serveroptimize = 1;
        }

        $stmt = $dbcon->prepare("UPDATE ServerInfo SET Lastserver = :last WHERE id = :id;");
		$stmt->bindParam(':last', $serveroptimize, PDO::PARAM_INT);
		$stmt->bindParam(':id', $lastserverid, PDO::PARAM_INT);
		$stmt->execute();

		// If we're still here, we can continue to request the server.
		$stmt = $dbcon->prepare("INSERT INTO serverRequests (`placeLocation`, `serverName`, `serverDescription`, `serverVersion`, `userID`, `serverPrivacy`, `server`) VALUES (:placeLocation, :serverName, :serverDescription, :version, :userID, :privacy, :server);");
		...
		$stmt->bindParam(':server', $serveroptimize, PDO::PARAM_INT);
		$stmt->execute();

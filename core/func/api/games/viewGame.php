<?php
	...
	if ($GLOBALS['loggedIn'] == false) {
		...
		if (file_exists("/var/www/api/imageServer/server/".$result['id'].".png") && $GLOBALS['loggedIn']) {
			echo "<img style=\"max-height:100%;max-width:100%\" src=\"http://labs.rhodum.xyz/imageServer/server/".$result['id'].".png?v=".strtotime($result['imgTime'])."\">";
		}
...
<script>
function join2008() {
    location.href = "https://rhodum.xyz/games/view/<?php echo $result['id']; ?>";
}
</script>
        <?php
		...
		if ($GLOBALS['loggedIn']) {
			...
			if ($result['version'] == 2) echo '<a class="btn btn-success fullWidth" href="RhodumClient3://'.$GLOBALS['userTable']['gameKey'].';'.$result['id'].';'.$GLOBALS['userTable']['id'].'">Play</a>';
		}else{

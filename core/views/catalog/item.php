		...
		<script src="/core/func/js/item.js?v=2"></script>
				<?php
					...
					if ($GLOBALS['loggedIn'] == false && $result['rbxasset'] == 1) {
						exit('<meta http-equiv="refresh" content="0; URL=https://rhodum.xyz/catalog/" />');
					}
				...
				<div class="well profileCard" style="margin-top:5px">
					<div class="row">
						...
						<div class="col-xs-6">
                            ...
                            if (context::secureString($result['stock']) == "inf"){
                                #haha
                            }else{
                                ?>
                                <p><b>Stock: </b><?php echo context::secureString($result['stock']); ?></p>
                                <?php
                            }
					...
					<?php
						if ($result['type'] == "decals") {
							...
							echo '<code style="word-wrap: break-word;white-space: pre-line;">http://rhodum.xyz/data/assets/uploads/'.$result['fileHash'].'</code><br>';
						}
					?>
					<b>Description</b><br>
					<?php

				...
				<div class="well profileCard center">
					...
					<?php
						if ($result['inGame'] == 1 and context::getOnline($result) == '<font color="green">&#x25CF; </font>') {
							if ($GLOBALS['loggedIn']) {
								...
								echo '<a href="RhodumClient://'.$GLOBALS['userTable']['gameKey'].';'.$resultGame['gameId'].';'.$GLOBALS['userTable']['id'].'" class="btn btn-info"><span class="fa fa-play"></span> In Game || Follow</a><br>';
							}else{
					...
					<?php
						if ($GLOBALS['loggedIn']) {
							if ($GLOBALS['userTable']['username'] != $result['username']) {
								...
								<a class="btn btn-primary" href="/user/messages+'.$result['username'].'"><span class="fa fa-envelope-open-o"></span> Message</a>';
					...
					<?php
						if (strlen($result['about']) == 0) {
                            ...
                            $testingshit = str_replace("(mycoins)", "Coins: ".$result['coins'], context::secureString($result['about']));
							echo '<span style="word-wrap:break-word;">'.context::secureString($testingshit).'</span>';
						}
					?>
					</p>
				</div>
					...
					<?php
						...
						foreach($stmt as $resultBadge) {
							if ($resultBadge['badgeId'] == 1) {
								$name = "Administrator";
								$description = "Owned by the administrators of Rhodum. Owners of this badge control the whole entire website and can change everything. Users who do not have this badge are not administrators.";
							}
							...
							if ($resultBadge['badgeId'] == 3) {
								$name = "Moderator";
								$description = "Owned by the moderators of Rhodum. Owners of this badge moderate the website. Users who do not have this badge are not moderators.";
							}
							...
							if ($resultBadge['badgeId'] == 5) {
								$name = "Member";
								$description = "This badge is owned by every single user of Rhodum and proves that you have played Rhodum.";
							}
							...
							if ($resultBadge['badgeId'] == 7) {
								$name = "Before 100";
								$description = "Owned by the very first 100 users of Rhodum. Users with this badge know the story of how we came to be.";
							}
							if ($resultBadge['badgeId'] == 8) {
								$name = "Gamer";
								$description = "This user has successfully joined a game at least once";
							}
							if ($resultBadge['badgeId'] == 9) {
								$name = "Builders Club";
								$description = "This user is a part of builders club.";
							}
							echo '<div data-toggle="tooltip" data-placement="top" data-original-title="'.$description.'" class="col-xs-12 col-sm-12 col-md-3 center"><img width="100" src="http://rhodum.xyz/html/img/badges/new/'.$resultBadge['badgeId'].'.png?v=5"><br><b>'.$name.'</b><br><br></div>';
							}
						?>
					</div>
				</div>
